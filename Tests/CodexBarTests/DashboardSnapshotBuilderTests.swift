...
struct DashboardSnapshotBuilderTests {
    ...
    func `builds stable display-oriented dashboard snapshot`() throws {
        ...
        let daily = try #require(costObject["daily"] as? [[String: Any]])
        #expect(daily.count == 1)
        #expect(daily[0]["date"] as? String == generatedDay)
        #expect(daily[0]["costUSD"] as? Double == 1.04)
        #expect(daily[0]["totalTokens"] as? Int == 1000)
